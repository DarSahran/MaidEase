...
export default function WashingClothesService() {
  ...
  // Laundry Load Size state
  const laundryLoadOptions = [
    { key: 'small', label: 'Small (up to 5kg)' },
    { key: 'medium', label: 'Medium (5–10kg)' },
    { key: 'large', label: 'Large (10kg+)' },
    { key: 'mixed', label: 'Mixed' },
  ];
  const [selectedLaundryLoads, setSelectedLaundryLoads] = useState<Record<string, boolean>>({});
  const handleLaundryLoadToggle = (key: string) => {
    setSelectedLaundryLoads((prev) => ({ ...prev, [key]: !prev[key] }));
  };
  ...
  // Detergent Option state
  const [detergentOption, setDetergentOption] = useState<'user' | 'maid'>('user');

  // Drying Method state
  const [dryingMethod, setDryingMethod] = useState<'sun' | 'machine' | 'none'>('none');
  ...
  const isFormValid = () => {
    return (
      selectedDate &&
      selectedTime &&
      Object.values(selectedLaundryLoads).some(Boolean) &&
      dryingMethod !== 'none' &&
      materialProvider
    );
  };
  ...
  const handleContinue = () => {
    ...
    const bookingData = {
      ...
      service_type: 'washing_clothes',
      address: selectedAddress,
      date: selectedDate ? selectedDate.toISOString() : '',
      time: selectedTime,
      laundryLoad: Object.keys(selectedLaundryLoads).filter((k) => selectedLaundryLoads[k]).join(', '),
      detergentOption,
      dryingMethod,
      materialProvider,
      notes,
      locationMethod,
      estimatedCost,
      duration_minutes: 60 // default duration for laundry
    };

    // Navigate to order summary (update route to bathroom_confirmation)
    router.push({
      pathname: '/(confirmation)/dusting_confirmation', // Change to your actual confirmation page for laundry if available
  ...
  useEffect(() => {
    let base = 70;
    // Laundry load size price
    const loadKeys = Object.keys(selectedLaundryLoads).filter((k) => selectedLaundryLoads[k]);
    let loadPrice = 0;
    if (selectedLaundryLoads['mixed']) {
      loadPrice = 30;
    } else {
      loadPrice = loadKeys.length * 10;
    }
    // Detergent fee
    let detergentFee = detergentOption === 'maid' ? 20 : 0;
    // Calculate total
    let total = Math.round(base + loadPrice + detergentFee);
    if (total < 20) total = 20;
    if (total > 120) total = 120;
    setEstimatedCost(total);
  }, [selectedLaundryLoads, detergentOption]);
      ...
      <ScrollView showsVerticalScrollIndicator={false}>
        ...
        <View style={styles.headerRow}>
          ...
          <View style={styles.headerTitleContainer}>
            <Text style={styles.headerTitle}>Book Your Laundry Service</Text>
          </View>
        </View>
        <View style={styles.content}>
          ...
          {/* Laundry Load Size Section */}
          <View style={styles.sectionHeader}>
            <Text style={styles.sectionTitle}>Laundry Load Size</Text>
          </View>
          <View style={styles.applianceList}>
            {laundryLoadOptions.map((load) => (
              <TouchableOpacity
                key={load.key}
                style={styles.applianceOption}
                onPress={() => handleLaundryLoadToggle(load.key)}
              >
                <View style={[
                  styles.checkbox,
                  selectedLaundryLoads[load.key] && styles.checkboxSelected
                ]}>
                  {selectedLaundryLoads[load.key] && (
          ...
          {/* Detergent Option Section */}
          <View style={styles.sectionHeader}>
            <Text style={styles.sectionTitle}>Detergent Option</Text>
          </View>
          <View style={styles.materialToggle}>
            <TouchableOpacity
              style={[
                styles.materialOption,
                detergentOption === 'user' && styles.materialOptionSelected
              ]}
              onPress={() => setDetergentOption('user')}
            >
              <Text style={[
                styles.materialOptionText,
                detergentOption === 'user' && styles.materialOptionTextSelected
              ]}>User Provides</Text>
            </TouchableOpacity>
            <TouchableOpacity
              style={[
                styles.materialOption,
                detergentOption === 'maid' && styles.materialOptionSelected
              ]}
              onPress={() => setDetergentOption('maid')}
            >
              <Text style={[
                styles.materialOptionText,
                detergentOption === 'maid' && styles.materialOptionTextSelected
              ]}>Maid Brings (+₹20)</Text>
            </TouchableOpacity>
          </View>

          {/* Drying Method Section */}
          <View style={styles.sectionHeader}>
            <Text style={styles.sectionTitle}>Drying Method</Text>
          </View>
          <View style={styles.materialToggle}>
            <TouchableOpacity
              style={[
                styles.materialOption,
                dryingMethod === 'sun' && styles.materialOptionSelected
              ]}
              onPress={() => setDryingMethod('sun')}
            >
              <Text style={[
                styles.materialOptionText,
                dryingMethod === 'sun' && styles.materialOptionTextSelected
              ]}>Sun Dry</Text>
            </TouchableOpacity>
            <TouchableOpacity
              style={[
                styles.materialOption,
                dryingMethod === 'machine' && styles.materialOptionSelected
              ]}
              onPress={() => setDryingMethod('machine')}
            >
              <Text style={[
                styles.materialOptionText,
                dryingMethod === 'machine' && styles.materialOptionTextSelected
              ]}>Machine Dry</Text>
            </TouchableOpacity>
            <TouchableOpacity
              style={[
                styles.materialOption,
                dryingMethod === 'none' && styles.materialOptionSelected
              ]}
              onPress={() => setDryingMethod('none')}
            >
              <Text style={[
                styles.materialOptionText,
                dryingMethod === 'none' && styles.materialOptionTextSelected
              ]}>No Preference</Text>
            </TouchableOpacity>
          </View>
