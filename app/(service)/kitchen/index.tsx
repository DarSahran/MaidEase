...
export default function KitchenService() {
  ...
  // Kitchen size state
  const [kitchenSize, setKitchenSize] = useState<'Small' | 'Medium' | 'Large' | null>(null);
  ...
  const handleApplianceToggle = (key: string) => {
    if (key === 'all') {
      const newValue = !selectedAppliances['all'];
      const updated: Record<string, boolean> = {};
      applianceOptions.forEach(opt => { updated[opt.key] = newValue; });
      setSelectedAppliances(updated);
    } else {
      const updated = { ...selectedAppliances, [key]: !selectedAppliances[key] };
      updated['all'] = applianceOptions.filter(a => a.key !== 'all').every(a => updated[a.key]);
      setSelectedAppliances(updated);
    }
  };

  // Grease level state
  const [greaseLevel, setGreaseLevel] = useState<'Low' | 'Medium' | 'High' | null>(null);
  const [greaseDropdownOpen, setGreaseDropdownOpen] = useState(false);
  ...
  const isFormValid = () => {
    return (
      ...
      greaseLevel &&
      materialProvider &&
      Object.values(selectedAppliances).some(Boolean)
    );
  };
  ...
  const handleContinue = () => {
    ...
    const bookingData = {
      ...
      time: selectedTime,
      kitchenSize,
      appliances: Object.keys(selectedAppliances).filter(k => selectedAppliances[k]),
      greaseLevel,
      materialProvider,
      notes,
      locationMethod,
    ...
    // Navigate to order summary (update route to kitchen_confirmation)
    router.push({
      pathname: '/(confirmation)/kitchen_confirmation',
  ...
  const getDefaultDuration = () => {
    if (kitchenSize === 'Large') return 90;
    if (kitchenSize === 'Medium') return 60;
    if (kitchenSize === 'Small') return 45;
    return 0;
  };
  ...
  }, [kitchenSize, selectedAppliances, greaseLevel, materialProvider, selectedTime]);

  // Calculate price based on kitchen size, appliances, grease level, and material provider
  function calculatePrice() {
    let price = 100;
    if (kitchenSize === 'Medium') price = 150;
    if (kitchenSize === 'Large') price = 200;
    // Add for appliances
    const applianceCount = Object.keys(selectedAppliances).filter(k => selectedAppliances[k] && k !== 'all').length;
    price += applianceCount * 20;
    // Grease level surcharge
    if (greaseLevel === 'Medium') price += 40;
    if (greaseLevel === 'High') price += 80;
    // Maid brings material surcharge
    if (materialProvider === 'maid') price += 50;
    // Demand multiplier
    let multiplier = 1;
    if (selectedDate && selectedTime) {
      ...
      if (isPM && hour !== 12) hour += 12;
      if (!isPM && hour === 12) hour = 0;
      if (day === 0 && hour >= 12) multiplier = 1.3;
      else if ((hour >= 7 && hour <= 10) || (hour >= 18 && hour <= 20)) multiplier = 1.15;
      else if (day === 6 || day === 0) multiplier = 1.1;
    }
      ...
      <ScrollView showsVerticalScrollIndicator={false}>
        ...
        <View style={styles.headerRow}>
          ...
          <View style={styles.headerTitleContainer}>
            <Text style={styles.headerTitle}>Book Your Kitchen Cleaning Service</Text>
          </View>
        </View>
        <View style={styles.content}>
          ...
          {/* Kitchen Size Section */}
          <View style={styles.sectionHeader}>
            <Text style={styles.sectionTitle}>Kitchen Size</Text>
          </View>
          <View style={{ flexDirection: 'row', gap: 12, marginBottom: 16 }}>
            {(['Small', 'Medium', 'Large'] as const).map((size) => (
              <TouchableOpacity
                key={size}
                style={[
                  styles.kitchenSizeOption,
                  kitchenSize === size && styles.kitchenSizeOptionSelected
                ]}
                onPress={() => setKitchenSize(size)}
              >
                <Text style={[
                  styles.kitchenSizeOptionText,
                  kitchenSize === size && styles.kitchenSizeOptionTextSelected
                ]}>{size}</Text>
              </TouchableOpacity>
            ))}
          </View>
          ...
          <View style={styles.applianceList}>
            {applianceOptions.map((appliance) => (
              <TouchableOpacity
                ...
                onPress={() => handleApplianceToggle(appliance.key)}
              >
                <View style={[
                  styles.checkbox,
                  selectedAppliances[appliance.key] && styles.checkboxSelected
                ]}>
                  {selectedAppliances[appliance.key] && (
                    <Ionicons name="checkmark" size={16} color="#121714" />
                  )}
                </View>
                <Text style={styles.applianceOptionText}>{appliance.label}</Text>
              </TouchableOpacity>
            ))}
          </View>

          {/* Grease Level Section */}
          <View style={styles.sectionHeader}>
            <Text style={styles.sectionTitle}>Grease Level</Text>
          </View>
          <View style={styles.dropdownRow}>
            <TouchableOpacity
              style={styles.dropdown}
              onPress={() => setGreaseDropdownOpen(!greaseDropdownOpen)}
            >
              <Text style={styles.dropdownText}>{greaseLevel || 'Select'}</Text>
              <Ionicons name={greaseDropdownOpen ? 'chevron-up' : 'chevron-down'} size={20} color="#638770" />
            </TouchableOpacity>
            {greaseDropdownOpen && (
              <View style={styles.dropdownMenu}>
                {(['Low', 'Medium', 'High'] as const).map((level) => (
                  <TouchableOpacity
                    ...
                    onPress={() => {
                      setGreaseLevel(level);
                      setGreaseDropdownOpen(false);
                    }}
                  >
          ...
          {/* Who provides materials Section */}
          <View style={styles.sectionHeader}>
            <Text style={styles.sectionTitle}>Who provides materials?</Text>
          </View>
          <View style={styles.materialToggle}>
            <TouchableOpacity
              style={[
                styles.materialOption,
                materialProvider === 'maid' && styles.materialOptionSelected
              ]}
              onPress={() => setMaterialProvider('maid')}
            >
              <Text style={[
                styles.materialOptionText,
                materialProvider === 'maid' && styles.materialOptionTextSelected
              ]}>
                Maid
              </Text>
            </TouchableOpacity>
            <TouchableOpacity
              style={[
                styles.materialOption,
                materialProvider === 'user' && styles.materialOptionSelected
              ]}
              onPress={() => setMaterialProvider('user')}
            >
              <Text style={[
                styles.materialOptionText,
                materialProvider === 'user' && styles.materialOptionTextSelected
              ]}>
                User
              </Text>
            </TouchableOpacity>
