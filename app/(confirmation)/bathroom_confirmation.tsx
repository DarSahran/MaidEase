...
export default function OrderSummary() {
  ...
  const [bathroomServiceId, setBathroomServiceId] = useState<string | null>(null);
  ...
  // Bathroom-specific helpers
  const getBathroomSizeText = () => bookingData.bathroomSize || 'Not Provided';
  const getStainsLevelText = () => bookingData.stainsLevel || 'Not Provided';
  const getMaterialProviderText = () => bookingData.materialProvider === 'maid' ? 'Maid' : 'User';
  const getFixturesText = () => {
    if (!bookingData.fixtures || !bookingData.fixtures.length) return 'Not Provided';
    return bookingData.fixtures.join(', ');
  };

  // Pricing logic (match bathroom/index.tsx if exists, else fallback)
  ...
  // Fetch the service_id for Bathroom Cleaning on mount
  useEffect(() => {
    const fetchBathroomServiceId = async () => {
      if (supabase) {
        const { data, error } = await supabase
          ...
          .single();
        if (data && data.id) setBathroomServiceId(data.id);
      }
    };
    fetchBathroomServiceId();
  }, [supabase]);

  const handleProceedToPayment = async () => {
    // Prepare service_details JSON
    const serviceDetails = {
      bathroomSize: bookingData.bathroomSize || '',
      stainsLevel: bookingData.stainsLevel || '',
      fixtures: bookingData.fixtures || [],
      materialProvider: bookingData.materialProvider || '',
      notes: bookingData.notes || ''
    };
    // Insert booking into Supabase
    const resolvedServiceId = bathroomServiceId || (bookingData.serviceId && bookingData.serviceId !== '' ? bookingData.serviceId : null);
    if (supabase) {
    ...
    router.push({
      ...
      params: {
        orderData: JSON.stringify({
          ...bookingData,
          service_id: bathroomServiceId || bookingData.serviceId || 'bathroom-default-id',
          service: 'Bathroom',
          service_type: 'bathroom',
          service_details: serviceDetails,
          costs,
  ...
  return (
    <SafeAreaView style={styles.container}>
      <ScrollView showsVerticalScrollIndicator={false}>
        ...
        <View style={styles.serviceRow}>
          ...
          <View style={styles.serviceDetails}>
            <Text style={styles.serviceName}>Bathroom Size</Text>
            <Text style={styles.serviceDescription}>{getBathroomSizeText()}</Text>
          </View>
        </View>
        ...
        <View style={styles.serviceRow}>
          ...
          <View style={styles.serviceDetails}>
            <Text style={styles.serviceName}>Fixtures</Text>
            <Text style={styles.serviceDescription}>{getFixturesText()}</Text>
          </View>
        </View>
        ...
        <View style={styles.serviceRow}>
          ...
          <View style={styles.serviceDetails}>
            <Text style={styles.serviceName}>Stains Level</Text>
            <Text style={styles.serviceDescription}>{getStainsLevelText()}</Text>
          </View>
        </View>
      ...
      <View style={styles.bottomSection}>
        <TouchableOpacity
          style={[styles.proceedButton, !bathroomServiceId && { opacity: 0.5 }]}
          onPress={handleProceedToPayment}
          disabled={!bathroomServiceId}
        >
